...
export default function ForumPost(props) {
  ...
  const commentsCards = comments.map((comment, index) =>
    ...
    //</ForumPost>
    <CommentCard key={index} user={comment.user} id={comment.id} ownerId={comment.ownerId} beans={comment.beans} myVote={comment.myVote} text={comment.text} commentType={"forum"} postId={id} postOwnerId={profileId}></CommentCard>
  );
